 ...
 */
public class Stare extends SimpleAttack {
    ...
    /**
     * creates new instance of {@link Stare} with given parameters
     *
     * @param commIds   {@link CommIds} for communication
     * @param actorId   monsterId of creature executing this stare-attacktest
     * @param direction {@link Direction} direction of the attacktest
     */
    public Stare(CommIds commIds, int actorId, Direction direction) {
    ...
    public void sendResults(ServerConnection serverConn) {
        for (int commId : super.commIDs.commLibIds) {
            serverConn.sendStared(commId, super.actorId, super.direction);
            if (super.targetDied) {
    ...
    public void sendCommand(ClientConnection cliConn) {
        cliConn.sendStare(super.direction);
    }
    ...
    public String getText(Client cli) {
        return String.format("%s stares at %s!", LogString.createVictimName(cli.getGame(),this.actorId),
