 ...
 * This test checks if only Kobold has access to the Slash attacktest - Please create an instance of this test for every other creature.
 */
public class SlashIllegalTest extends OurSystemTest {
    private CreatureType t;

    public SlashIllegalTest(CreatureType t) {
        super("SlashIllegalTest" + t.toString());
        this.t = t;
    }

    @Override
    protected void executeTest() {
        ClientConnection<Event> legalCreature = register(0, "Hobgoblin", CreatureType.KOBOLD, "TeamLegal", 0, 0);
        ClientConnection<Event> illegalCreature = register(1, "ein" + t.toString(), t, "TeamIllegal", 1, 0);
        assertRegisterEvent(illegalCreature.nextEvent(), 0, "Hobgoblin", CreatureType.KOBOLD, "TeamLegal", 0, 0);
        ...
        legalCreature.sendSlash(Direction.EAST); //LegalCreature slashes IllegalCreature
        assertSlashed(assertAndMerge(legalCreature, illegalCreature), 0, Direction.EAST);
        ...
        illegalCreature.sendSlash(Direction.WEST); //IllegalCreature tries to slash back
